    ...
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg, var(--navy) 0%, #1a3a5c 60%, #2d5a8e 100%)', display:'flex', alignItems:'center', justifyContent:'center', padding:'1.5rem' }}>
      <div style={{ background:'white', borderRadius:'1.25rem', padding:'2.5rem', width:'100%', maxWidth:'480px', boxShadow:'0 24px 80px rgba(0,0,0,0.3)' }}>
        <div style={{ textAlign:'center', marginBottom:'1.75rem' }}>
          ...
          <h1 style={{ fontSize:'1.4rem', margin:'0 0 0.25rem', color:'var(--navy)' }}>{t('createAccount')}</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ marginBottom:'1.5rem' }}>
                ...
                <div style={{ position:'relative' }}>
                  <input type={showPw ? 'text' : 'password'} className="input" value={form.password}
                    onChange={e => set('password', e.target.value)} placeholder={t('passwordHint')}
              ...
              <button type="button" className="btn btn-navy btn-lg" style={{ width:'100%' }}
                onClick={() => { if (!form.fullName || !form.email || form.password.length < 8) { setError(t('fillAllFields')) } else { setError(''); setStep(2) } }}>
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{ marginBottom:'1.5rem' }}>
                <label className="label">{t('operatorTypeQuestion')}</label>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginTop:'0.375rem' }}>
                  {OPERATOR_TYPES.map(ot => (
                    <button key={ot.value} type="button"
                      onClick={() => set('operatorType', ot.value)}
                      style={{
                        padding:'0.625rem 0.75rem', borderRadius:'0.5rem', border:'1.5px solid',
                        borderColor: form.operatorType === ot.value ? 'var(--gold)' : 'var(--gray-200)',
                        background: form.operatorType === ot.value ? 'var(--gold-50)' : 'white',
                        cursor:'pointer', fontSize:'0.8125rem', fontWeight: form.operatorType === ot.value ? 600 : 400,
                        color: form.operatorType === ot.value ? 'var(--navy)' : 'var(--gray-600)',
              ...
              <div style={{ display:'flex', gap:'0.75rem' }}>
                ...
                <button type="button" className="btn btn-navy btn-lg" style={{ flex:2 }}
                  onClick={() => { if (!form.companyName) { setError(t('enterBusinessName')) } else { setError(''); setStep(3) } }}>
                  {t('continue')} <ChevronRight size={16} />
                </button>
              </div>
            </div>
          ...
          {step === 3 && (
            <div>
              <div style={{ marginBottom:'1.5rem' }}>
                <label className="label">{t('billingCurrency')}</label>
                <p style={{ fontSize:'0.8125rem', color:'var(--gray-400)', margin:'0 0 0.5rem' }}>
                  {t('billingCurrencyHint')}
                </p>
              ...
              <div style={{ display:'flex', gap:'0.75rem' }}>
                ...
                <button type="submit" className="btn btn-primary btn-lg" style={{ flex:2 }} disabled={loading}>
                  {loading ? t('creatingAccount') : <>{t('createAccountButton')} <BrandIcon name="arrowRight" size={14} /></>}
                </button>
              </div>
            </div>
          )}
        ...
        <div style={{ textAlign:'center', marginTop:'1.5rem', paddingTop:'1.5rem', borderTop:'1px solid var(--gray-100)' }}>
          <p style={{ fontSize:'0.8125rem', color:'var(--gray-400)', margin:0 }}>
            ...
            <Link href="/auth/login" style={{ color:'var(--orange)', fontWeight:600, textDecoration:'none' }}>{t('signIn')}</Link>
          </p>
        </div>
      </div>
    </div>
