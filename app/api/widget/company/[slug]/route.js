...
export async function GET(request, { params }) {
  ...
  const { data: company } = await supabase
    .from('companies')
    .select('id, name, logo_url, currency, operator_type, widget_enabled')
    .eq('slug', slug)
    .maybeSingle()
  ...
  return NextResponse.json({
    company: { name: company.name, logo_url: company.logo_url, currency: company.currency, operator_type: company.operator_type },
