  ...
  return (
    <nav style={{ background:'var(--navy)', padding:'0 2rem', height:'4rem', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100 }}>
      ...
      <div style={{ display:'flex', gap:'1.5rem', alignItems:'center' }}>
        {[['/features', t('features')], ['/pricing', t('pricing')], ['/about', t('about')]].map(([href, label]) => (
        ...
        <LanguageSwitcher />
        <Link href="/auth/login" style={{ color:'rgba(255,255,255,0.7)', textDecoration:'none', fontSize:'0.875rem', fontWeight:500 }}>{t('signIn')}</Link>
        <Link href="/auth/signup" className="btn btn-primary btn-sm">{t('startFree')} <BrandIcon name="arrowRight" size={14} style={{ display:'inline-block' }} /></Link>
      </div>
    </nav>
  )
}
