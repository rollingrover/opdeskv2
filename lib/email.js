...
export async function sendEmail({ to, subject, html, bcc, attachments }) {
  ...
  if (attachments) payload.attachments = attachments // [{ filename, content (base64) }]
  ...
  return `
    <div style="font-family: Inter, -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px;">
      ...
      <p style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #eee; color: #9ca3af; font-size: 12px;">
        OpDesk (Pty) Ltd — Operate. Explore. Grow.<br/>
        <span style="color: #c1c7cf; font-size: 11px;">Reg. No. 2026/648649/07 · B-BBEE Level 4 Contributor</span>
      </p>
    </div>
  `
}
...
export const EMAIL_TEMPLATES = {
  ...
  widget_booking_request: ({ toEmail, companyName, guestName, bookingTypeName, startDate, guestCount }) => ({
    to: toEmail,
    subject: `New booking request from your website — ${guestName}`,
    html: wrap(`
      <p>A new booking request came in through your website's booking widget.</p>
      <p><strong>Guest:</strong> ${guestName}<br/>
         <strong>Type:</strong> ${bookingTypeName}<br/>
         <strong>Date:</strong> ${startDate}<br/>
         <strong>Guests:</strong> ${guestCount}</p>
      <p>It's waiting in your Bookings list as a pending request — confirm the date works and get in touch with the guest to finalise it.</p>
      <p><a href="${SITE_URL}/bookings" style="color:#D4A853;">View in Bookings →</a></p>
    `, { heading: `New booking request — ${companyName}` }),
  }),

  team_invite: ({ toEmail, companyName, inviterName, role, token }) => ({
    to: toEmail,
    subject: `${inviterName} invited you to join ${companyName} on OpDesk`,
    html: wrap(`
      <p>${inviterName} has invited you to join <strong>${companyName}</strong> on OpDesk as a <strong>${role === 'admin' ? 'team admin' : 'team member'}</strong>.</p>
      <p>OpDesk is how ${companyName} manages bookings, staff, fleet and invoicing — accept the invite to get access.</p>
      <p><a href="${SITE_URL}/auth/accept-invite?token=${token}" style="display:inline-block;background:#D4A853;color:#0F2540;padding:0.75rem 1.5rem;border-radius:0.5rem;text-decoration:none;font-weight:700;">Accept Invite</a></p>
      <p style="font-size:0.8125rem;color:#6b7280;">If you weren't expecting this, you can safely ignore this email.</p>
